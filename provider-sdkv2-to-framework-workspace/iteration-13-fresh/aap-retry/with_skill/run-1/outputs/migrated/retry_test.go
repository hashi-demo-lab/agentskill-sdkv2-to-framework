...
func TestRetryOperation(t *testing.T) {
	...
	t.Run("operation times out after multiple retries", func(t *testing.T) {
		...
		mockOp.EXPECT().Execute().Return(nil, diag.Diagnostics{}, http.StatusConflict).MinTimes(2).MaxTimes(15)
...
// TestWaitForState exercises the inline waitForState ticker loop directly.
// This replaces test coverage that used to come transitively from
// retry.StateChangeConf.WaitForStateContext.
func TestWaitForState(t *testing.T) {
	t.Parallel()

	t.Run("returns target value on first refresh", func(t *testing.T) {
		t.Parallel()
		v, err := waitForState(
			context.Background(),
			func() (any, string, error) { return "ready", RetryStateSuccess, nil },
			[]string{RetryStateRetrying},
			[]string{RetryStateSuccess},
			1*time.Millisecond,
	...
	t.Run("polls until target reached", func(t *testing.T) {
		t.Parallel()
		calls := 0
		v, err := waitForState(
			context.Background(),
			func() (any, string, error) {
				calls++
				if calls < 3 {
					return calls, RetryStateRetrying, nil
				}
				return calls, RetryStateSuccess, nil
			},
			[]string{RetryStateRetrying},
			[]string{RetryStateSuccess},
			1*time.Millisecond,
	...
	t.Run("returns error when refresh returns error", func(t *testing.T) {
		t.Parallel()
		boom := fmt.Errorf("boom")
		_, err := waitForState(
			context.Background(),
			func() (any, string, error) { return nil, "", boom },
			[]string{RetryStateRetrying},
			[]string{RetryStateSuccess},
			1*time.Millisecond,
			100*time.Millisecond,
	...
	t.Run("returns error on unexpected state", func(t *testing.T) {
		...
		_, err := waitForState(
			context.Background(),
			func() (any, string, error) { return nil, "weird", nil },
			[]string{RetryStateRetrying},
			[]string{RetryStateSuccess},
			1*time.Millisecond,
			100*time.Millisecond,
	...
	t.Run("returns error on timeout", func(t *testing.T) {
		...
		_, err := waitForState(
			context.Background(),
			func() (any, string, error) { return nil, RetryStateRetrying, nil },
			[]string{RetryStateRetrying},
			[]string{RetryStateSuccess},
			1*time.Millisecond,
			10*time.Millisecond,
