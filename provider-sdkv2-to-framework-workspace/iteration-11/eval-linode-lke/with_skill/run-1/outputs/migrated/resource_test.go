...
func checkLKEExists(cluster *linodego.LKECluster) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		client := acceptance.TestAccProvider.Meta().(*helper.FrameworkProviderMeta).Meta.Client
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				},
				{
					Config: tmpl.DataTaintsLabels(
						t, clusterName, k8sVersionLatest, testRegion, []tmpl.TaintData{
							{Effect: "PreferNoSchedule", Key: "baz", Value: "qux"},
						},
...
func TestAccResourceLKECluster_timeouts(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Ensure the timeouts block is accepted by the schema without error.
				Config: fmt.Sprintf(`
...
`, clusterName, k8sVersionLatest, testRegion),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceClusterName, "label", clusterName),
				),
			},
...
func TestAccResourceLKECluster_sensitiveKubeconfig(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: tmpl.Basic(t, clusterName, k8sVersionLatest, testRegion),
				// kubeconfig is Sensitive: true; the state value is populated but
				// the plan output masks it. Verify it's set (non-empty).
				ConfigStateChecks: []statecheck.StateCheck{
