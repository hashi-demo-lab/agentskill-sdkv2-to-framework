...
import (
	...
	"github.com/linode/terraform-provider-linode/v3/linode/domain/tmpl"
)
...
func checkDomainExists(s *terraform.State) error {
	// Use ProtoV6ProviderFactories – retrieve client via the framework provider meta.
	// For acceptance tests, we obtain a linodego client via acceptance.GetTestClient().
	client, err := acceptance.GetTestClient()
	if err != nil {
