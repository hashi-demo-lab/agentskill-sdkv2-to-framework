...
func checkBucketExists(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		regionOrCluster, label, err := objbucket.DecodeBucketID(context.Background(), rs.Primary.ID)
		if err != nil {
...
func checkBucketDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		id := rs.Primary.ID
		regionOrCluster, label, err := objbucket.DecodeBucketID(context.Background(), id)
		if err != nil {
