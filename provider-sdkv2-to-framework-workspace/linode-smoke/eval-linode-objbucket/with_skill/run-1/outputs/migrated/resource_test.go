...
// TestSmokeTests_objbucket groups the smoke tests to allow running them together.
	...
	acceptance.RunTestWithRetries(t, 5, func(t *acceptance.WrappedT) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				{
					// Import with composite ID "cluster:label"
					ResourceName:      resName,
					ImportState:       true,
					ImportStateVerify: true,
					// cert fields are write-only (not returned by the API).
	...
	acceptance.RunTestWithRetries(t, 5, func(t *acceptance.WrappedT) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				{
					// Composite ID import: the ID stored in state is "<region>:<label>".
					ResourceName:            resName,
					ImportState:             true,
					ImportStateVerify:       true,
					ImportStateVerifyIgnore: []string{"cert"},
	...
	acceptance.RunTestWithRetries(t, 5, func(t *acceptance.WrappedT) {
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				{
					// Remove cert block — bucket remains but cert is deleted.
					Config: tmpl.Basic(t, objectStorageBucketName, testRegion),
					Check: resource.ComposeTestCheckFunc(
						checkBucketExists,
						resource.TestCheckResourceAttr(resName, "label", objectStorageBucketName),
...
func checkBucketExists(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		// The framework resource stores ID as "<region_or_cluster>:<label>".
		id := rs.Primary.ID
		parts := strings.SplitN(id, ":", 2)
		if len(parts) != 2 {
			return fmt.Errorf("Error parsing %s: expected '<region_or_cluster>:<label>'", id)
		}
		regionOrCluster, label := parts[0], parts[1]

		_, err := client.GetObjectStorageBucket(context.Background(), regionOrCluster, label)
		if err != nil {
...
func checkBucketDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if label == "" {
			return fmt.Errorf("Would have considered %s as empty label", id)
		}
