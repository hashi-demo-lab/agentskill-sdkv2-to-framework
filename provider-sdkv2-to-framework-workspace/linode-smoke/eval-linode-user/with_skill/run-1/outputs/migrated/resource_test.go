		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					// global_grants is now a block — indices still work the same way
					// in terraform-plugin-testing attribute path notation.
...
func checkUserDestroy(s *terraform.State) error {
	// Use the framework provider's meta for the destroy check.
