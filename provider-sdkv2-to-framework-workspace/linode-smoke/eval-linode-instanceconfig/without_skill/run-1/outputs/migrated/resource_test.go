...
func TestAccResourceInstanceConfig_deviceBlock(t *testing.T) {
	...
	// Note: in the framework migration the "device" block is a set, not an ordered list.
	// Checks for indexed positions (device.0, device.1) are replaced with set-aware checks.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerify: true,
				ImportStateIdFunc: resourceImportStateID,
				// The "device" block is a Set in the framework; ordering is non-deterministic.
				// ImportStateVerify for sets may produce spurious diffs — ignore for now.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerify: true,
				ImportStateIdFunc: resourceImportStateID,
				// The "device" set may produce ordering diffs on import.
				ImportStateVerifyIgnore: []string{"device"},
			},
		},
	})
