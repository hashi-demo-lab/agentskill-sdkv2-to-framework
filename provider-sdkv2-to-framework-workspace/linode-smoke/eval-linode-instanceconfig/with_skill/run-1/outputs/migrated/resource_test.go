...
func TestAccResourceInstanceConfig_deviceBlock(t *testing.T) {
	...
	// Note: `devices.0.sda.0.disk_id` uses list-of-block path notation (preserved for backward compat).
	// `device` uses set path notation; set elements may be in any order.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerify: true,
				ImportStateIdFunc: resourceImportStateID,
				// `device` is a set; ordering is non-deterministic on import vs. prior state.
...
// TestAccResourceInstanceConfig_rescueBooted ensures instances manually booted
// into rescue mode will not crash the provider.
