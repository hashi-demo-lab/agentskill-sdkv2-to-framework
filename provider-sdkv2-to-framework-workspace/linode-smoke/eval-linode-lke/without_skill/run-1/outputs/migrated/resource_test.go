...
// checkLKEExists retrieves the LKE cluster from the API and populates cluster.
// Uses a standalone client rather than the provider meta so it works with both
// SDKv2 and framework providers.
...
// waitForAllNodesReady waits for every Node in every NodePool to be in a ready state.
