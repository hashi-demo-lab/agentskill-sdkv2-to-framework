...
func init() {
	...
	v, err = databaseshared.ResolveValidDBEngine(context.Background(), *client, "postgresql")
	if err != nil {
		log.Fatalf("failed to get db engine version: %s", err)
	}
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					checkMySQLDatabaseExists,
					resource.TestCheckResourceAttr(resName, "allow_list.#", "1"),
					resource.TestCheckTypeSetElemAttr(resName, "allow_list.*", "192.168.0.25/32"),
				),
			},
			{
...
// resourceImportStateID builds the composite import ID "<database_id>:<database_type>"
// used by the framework resource's ImportState method.
func resourceImportStateID(s *terraform.State) (string, error) {
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "linode_database_access_controls" {
			continue
		}
		return rs.Primary.ID, nil
	}
	return "", fmt.Errorf("linode_database_access_controls resource not found in state")
}
