...
func TestAccLBV2Member_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// Switch from ProviderFactories (SDKv2) to ProtoV6ProviderFactories
		// now that this resource is served via the framework.
		...
		Steps: []resource.TestStep{
			{
				...
				// Assert that pool_id (part of the identity) is set in state.
			...
			// Import via legacy composite-ID string: <pool_id>/<member_id>.
			// This exercises the req.ID != "" branch in ImportState.
			{
				ResourceName:      "openstack_lb_member_v2.member_1",
				ImportState:       true,
				ImportStateVerify: true,
				ImportStateIdFunc: testAccLBV2MemberImportID("openstack_lb_member_v2.member_1"),
			},
		},
	})
}
...
// testAccLBV2MemberImportID builds an ImportStateIdFunc that returns the legacy
// composite import string "<pool_id>/<member_id>" so that ImportState steps work
// without requiring the practitioner to know the format.
func testAccLBV2MemberImportID(resourceName string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		if poolID == "" || memberID == "" {
			return "", errors.New("pool_id or member_id is empty — cannot construct import ID")
		}

		return poolID + "/" + memberID, nil
	}
}
