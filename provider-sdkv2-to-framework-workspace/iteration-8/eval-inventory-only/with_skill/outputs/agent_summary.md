...
| File | Description |
|------|-------------|
| `audit_report.md` | Verbatim output of `audit_sdkv2.sh` run against `<openstack-clone>` |
...
- **Pre-flight A (audit):** `bash sdkv2-to-framework/scripts/audit_sdkv2.sh <openstack-clone>` completed successfully using semgrep AST-aware matching. SDKv2 version confirmed as `v2.38.1`.
