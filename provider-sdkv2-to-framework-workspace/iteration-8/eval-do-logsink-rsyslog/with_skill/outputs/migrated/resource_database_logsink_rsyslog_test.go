...
func TestAccDigitalOceanDatabaseLogsinkRsyslog_Basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Import round-trip: composite ID is cluster_id,logsink_id
			{
				ResourceName:      "digitalocean_database_logsink_rsyslog.test",
				ImportState:       true,
				ImportStateVerify: true,
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					rs := s.RootModule().Resources["digitalocean_database_logsink_rsyslog.test"]
					return fmt.Sprintf("%s,%s", rs.Primary.Attributes["cluster_id"], rs.Primary.Attributes["logsink_id"]), nil
				},
			},
		},
	})
...
// ---------------------------------------------------------------------------
// Test configurations (HCL templates)
// ---------------------------------------------------------------------------
