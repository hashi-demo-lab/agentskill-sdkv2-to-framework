...
func TestAccResourceLKECluster_basicUpdates(t *testing.T) {
	...
	// Note: The SDKv2 version of this test intercepted PUT requests to verify
	// k8s_version is excluded when unchanged. The framework resource achieves
	// the same via the conditional hasClusterUpdate guard in Update(). This
	// test validates the externally-visible behavior (label/tags/count updates).
	acceptance.RunTestWithRetries(t, 2, func(t *acceptance.WrappedT) {
		clusterName := acctest.RandomWithPrefix("tf_test")
	...
	acceptance.RunTestWithRetries(t, 2, func(t *acceptance.WrappedT) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				{
					PreConfig: func() {
						client, clientErr := acceptance.GetTestClient()
						if clientErr != nil {
							t.Errorf("failed to get test client: %s", clientErr)
							return
						}
