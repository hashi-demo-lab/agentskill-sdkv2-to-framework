		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				},
				{
					...
					// secret_key and access_key are write-only and cannot be read back.
					ImportStateVerifyIgnore: []string{"secret_key", "access_key"},
				},
			},
		})
	})
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						resource.TestCheckResourceAttrSet(resName, "lifecycle_rule.0.expiration.date"),
					),
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						resource.TestCheckResourceAttr(resName, "lifecycle_rule.0.expiration.days", "37"),
					),
				},
				{
...
// checkBucketExists verifies the bucket exists via the API.
func checkBucketExists(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		cluster, label, err := objbucket.DecodeBucketIDString(context.Background(), rs.Primary.ID)
		if err != nil {
...
// checkBucketDestroy verifies the bucket was destroyed.
func checkBucketDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		id := rs.Primary.ID
		cluster, label, err := objbucket.DecodeBucketIDString(context.Background(), id)
		if err != nil {
