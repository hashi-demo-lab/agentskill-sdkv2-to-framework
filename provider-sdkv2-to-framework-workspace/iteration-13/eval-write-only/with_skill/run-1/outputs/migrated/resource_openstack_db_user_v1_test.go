	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				// "password" is a WriteOnly attribute — it is supplied by the
				// practitioner in config but not persisted to state, so it is
				// null in the imported state and must be excluded from
				// import-verify comparison.
