...
// All tests in this file rely on ProtoV6ProviderFactories — the framework
// path. The legacy SDKv2 ProviderFactories field is intentionally absent;
// leaving it set is the most common silent-regression after a migration
// because tests would still wire through the SDKv2 server even though the
// resource is now framework-served.
...
func TestAccResourceUser_grants(t *testing.T) {
	...
	email := username + "@example.com"
	// global_grants is preserved as a (list-shaped) block, so the
	// "global_grants.0.<field>" state paths still apply.
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { acceptance.PreCheck(t) },
		ProtoV6ProviderFactories: acceptance.ProtoV6ProviderFactories,
		CheckDestroy:             checkUserDestroy,
...
// checkUserDestroy uses the SDKv2 provider only as a convenient source of an
// authenticated linodego client for direct API checks against destroyed
// resources. It does NOT cause the linode_user resource (which is now
// framework-served) to be routed through the SDKv2 plumbing.
