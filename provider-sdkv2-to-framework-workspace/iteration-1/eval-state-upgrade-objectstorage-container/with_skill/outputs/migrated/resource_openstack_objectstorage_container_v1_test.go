...
//   - Acceptance tests are otherwise structurally unchanged; resource
//     addresses and HCL configs are identical.
//
// State-upgrade unit test (replaces migrate_resource_openstack_objectstorage_container_v1_test.go):
//   - The SDKv2 test called resourceObjectStorageContainerStateUpgradeV0
//     directly on a raw map.  The framework unit test instantiates the
//     resource, calls UpgradeState, and exercises the typed upgrader function
//     giving compile-time type safety and proving the PriorSchema / model tags
//     are correct.
...
// that directly exercises upgradeContainerStateV0toV1.
//
// It constructs a fake V0 state using the framework's tfsdk.State helpers,
// calls the upgrader, and asserts the resulting V1 state has the expected
// field values.
//
// Unlike the old SDKv2 test which operated on raw map[string]any, this test
// is fully typed — a mismatch between the PriorSchema and the V0 model struct
// tags will cause a test failure at the Get() call rather than silent data
// loss.
func TestObjectStorageV1ContainerStateUpgradeV0(t *testing.T) {
	...
	// Sanity-check: force_destroy false.
	if result.ForceDestroy.ValueBool() {
		t.Errorf("expected force_destroy=false, got true")
	}

	// Unused import guard: ensure types is referenced.
	_ = types.StringNull()
}
