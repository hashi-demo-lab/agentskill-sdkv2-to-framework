...
func (d *blockStorageAvailabilityZonesV3DataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_blockstorage_availability_zones_v3"
}

func (d *blockStorageAvailabilityZonesV3DataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *blockStorageAvailabilityZonesV3DataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *blockStorageAvailabilityZonesV3DataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg blockStorageAvailabilityZonesV3Model
	resp.Diagnostics.Append(req.Config.Get(ctx, &cfg)...)
	if resp.Diagnostics.HasError() {
		return
	}

	// Resolve region: prefer the value from config, fall back to provider-level region.
	region := d.config.Region
	if !cfg.Region.IsNull() && !cfg.Region.IsUnknown() && cfg.Region.ValueString() != "" {
		region = cfg.Region.ValueString()
	}

	// Resolve state default: "available" when not set.
	stateVal := "available"
	if !cfg.State.IsNull() && !cfg.State.IsUnknown() && cfg.State.ValueString() != "" {
		stateVal = cfg.State.ValueString()
	}
	...
	cfg.State = types.StringValue(stateVal)
	cfg.Names = namesList
