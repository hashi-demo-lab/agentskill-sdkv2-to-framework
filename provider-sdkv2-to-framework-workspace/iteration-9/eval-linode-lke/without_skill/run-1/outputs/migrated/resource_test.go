...
func checkLKEExists(cluster *linodego.LKECluster) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the framework provider meta instead of SDKv2 provider meta
...
func TestAccResourceLKECluster_basicUpdates(t *testing.T) {
	...
	// With framework, we use the ProtoV6ProviderFactories and intercept via client hooks.
	acceptance.RunTestWithRetries(t, 2, func(t *acceptance.WrappedT) {
		clusterName := acctest.RandomWithPrefix("tf_test")
		newClusterName := acctest.RandomWithPrefix("tf_test")

		// Validate that k8s_version is not included in update requests when not changed.
		validateNoK8sVersionInUpdate := func() {
			client, err := acceptance.GetTestClient()
			if err != nil {
		...
		_ = validateNoK8sVersionInUpdate // register for completeness; framework handles this natively
			...
			Steps: []resource.TestStep{
				...
				},
				{
					PreConfig: func() {
						client, err := acceptance.GetTestClient()
						if err != nil {
							t.Errorf("failed to get test client: %s", err)
						}
...
func TestAccResourceLKECluster_tierNoAccess(t *testing.T) {
	...
	apiVersionOverrideProvider := acceptance.ModifyProviderMeta(
		linode.Provider(),
		func(ctx context.Context, data *helper.FrameworkProviderMeta, config *helper.ProviderMeta) error {
