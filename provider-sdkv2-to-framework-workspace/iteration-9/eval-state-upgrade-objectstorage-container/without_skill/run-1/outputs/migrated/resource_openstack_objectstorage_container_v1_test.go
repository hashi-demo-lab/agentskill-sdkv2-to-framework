...
// TestAccObjectStorageV1Container_stateUpgradeV0 verifies the state upgrader
// moves the versioning set to versioning_legacy and sets versioning to false.
func TestAccObjectStorageV1Container_stateUpgradeV0(t *testing.T) {
	...
	v1State := map[string]any{
		"name":              v0State["name"],
		"versioning":        false,
		"versioning_legacy": v0State["versioning"],
	}

	// Verify that the upgrade function produces the expected result using the
	// existing unit-test helper from the SDKv2 migrate file (kept for
	// compatibility during the transition period).
	actual, err := resourceObjectStorageContainerStateUpgradeV0(context.Background(), v0State, nil)
	if err != nil {
		t.Fatalf("error migrating state: %s", err)
	}

	if fmt.Sprintf("%v", actual["versioning"]) != fmt.Sprintf("%v", v1State["versioning"]) {
		t.Errorf("versioning mismatch: got %v, want %v", actual["versioning"], v1State["versioning"])
	}

	if fmt.Sprintf("%v", actual["versioning_legacy"]) != fmt.Sprintf("%v", v1State["versioning_legacy"]) {
		t.Errorf("versioning_legacy mismatch: got %v, want %v", actual["versioning_legacy"], v1State["versioning_legacy"])
	}
}
