		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				},
				{
					...
					// cert.private_key and cert.certificate are write-only; cannot be read back.
					ImportStateVerifyIgnore: []string{"cert.0.private_key", "cert.0.certificate"},
				},
			},
		})
	})
			...
			Steps: []resource.TestStep{
				...
				},
				{
					PreConfig: func() {
						client, err := acceptance.GetTestClient()
						if err != nil {
							t.Errorf("error getting test client in PreConfig func: %v", err)
							return
						}
