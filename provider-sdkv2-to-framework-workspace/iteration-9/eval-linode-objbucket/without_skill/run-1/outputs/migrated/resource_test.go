	...
	acceptance.RunTestWithRetries(t, 5, func(t *acceptance.WrappedT) {
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				{
					PreConfig: func() {
						// Use the framework provider meta (Meta is a field, not a method).
...
func checkBucketExists(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		regionOrCluster, label, err := objbucket.DecodeBucketIDString(rs.Primary.ID)
		if err != nil {
...
func checkBucketDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		id := rs.Primary.ID
		regionOrCluster, label, err := objbucket.DecodeBucketIDString(id)
		if err != nil {
