		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// password is WriteOnly — must be null in state.
			...
			},
			{
				...
				// password is Sensitive + WriteOnly: never persisted to
				// state, so the importer cannot round-trip it. Excluding
				// it from ImportStateVerify is mandatory — without this
				// the step fails with "expected x, got null". See
