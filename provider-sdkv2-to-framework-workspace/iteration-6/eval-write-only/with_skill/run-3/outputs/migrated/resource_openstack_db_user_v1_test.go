...
func TestAccDatabaseV1User_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// Provider is now framework-based; use protocol v6 factories.
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// password is WriteOnly: it is NOT persisted in state.
					// Asserting null guards against accidental persistence.
			...
			},
			{
				...
				// password is WriteOnly: the importer cannot read it back,
				// so it must be excluded from the import-verify diff.
