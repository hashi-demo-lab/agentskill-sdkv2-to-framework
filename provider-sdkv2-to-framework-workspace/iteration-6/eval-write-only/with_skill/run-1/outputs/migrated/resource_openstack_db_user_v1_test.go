...
func TestAccDatabaseV1User_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// Switched from SDKv2 ProviderFactories to framework
		// ProtoV6ProviderFactories. testAccProtoV6ProviderFactories must be
		// declared in the provider's test bootstrap (see provider_test.go)
		// alongside the existing testAccProviders during migration.
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// password is write-only, so it MUST NOT appear in state.
					// Asserting it as null guards against a regression where
					// the attribute is accidentally re-added with Computed
					// (which would cause Terraform to materialise a value).
			...
			},
			{
				...
				// password is WriteOnly: it is never persisted to state, so
				// after import the attribute is null. ImportStateVerify must
				// ignore it or the verification step will fail because the
				// imported value (null) does not equal the configured value.
