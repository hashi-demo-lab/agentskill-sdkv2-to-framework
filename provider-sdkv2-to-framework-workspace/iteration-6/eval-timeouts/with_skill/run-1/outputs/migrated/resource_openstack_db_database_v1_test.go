...
func TestAccDatabaseV1Database_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// SDKv2 ProviderFactories → framework ProtoV6ProviderFactories.
		// This variable is defined at the provider level (provider_test.go)
		// once the provider itself has been wired to protocol v6 (or muxed).
		// See notes.md for the required provider-level glue.
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// Verify the timeouts block round-trips through state.
					resource.TestCheckResourceAttr(
						"openstack_db_database_v1.basic", "timeouts.create", "20m"),
					resource.TestCheckResourceAttr(
						"openstack_db_database_v1.basic", "timeouts.delete", "15m"),
				),
			},
			{
				...
				// timeouts is a config-only block; it is not stored on the
				// remote and so cannot be reproduced after import.
