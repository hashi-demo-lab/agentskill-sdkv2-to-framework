...
func TestAccComputeV2Keypair_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// NOTE: openstack_compute_keypair_v2 has been migrated to
		// terraform-plugin-framework. The rest of the provider remains on
		// SDKv2 so the wider acceptance suite still references
		// ProviderFactories. Once provider muxing is wired up
		// (terraform-plugin-mux + ProtoV6ProviderFactories) this single
		// declaration can be replaced with the muxed factory.
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				// private_key is only available in the create response, so
				// it will not match on re-import.
