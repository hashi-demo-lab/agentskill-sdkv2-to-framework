...
// regexpConflictsWithPortNetwork matches the framework's standard
// stringvalidator.ConflictsWith error message ("Attribute ... cannot be
// specified when ... is specified"). Anchoring on "cannot be specified when"
// keeps the test resilient to message wording tweaks.
var regexpConflictsWithPortNetwork = regexp.MustCompile(`cannot be specified when`)
...
func TestAccComputeV2InterfaceAttach_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// for the framework-served resource. testAccProtoV6ProviderFactories
		// is the provider-wide factory map that returns
		// providerserver.NewProtocol6WithError(NewProvider()).
...
// TestAccComputeV2InterfaceAttach_conflictingPortAndNetwork exercises the new
// per-attribute ConflictsWith validators. It expects the plan to error during
// configuration validation (no Apply), proving the framework validator fires.
func TestAccComputeV2InterfaceAttach_conflictingPortAndNetwork(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccComputeV2InterfaceAttachConflict(),
				ExpectError: regexpConflictsWithPortNetwork,
				PlanOnly:    true,
			},
		},
	})
...
func testAccCheckComputeV2InterfaceAttachDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// In the framework-only world, the SDKv2 testAccProvider singleton
		// is gone. The test relies on the same Config the provider-factory
		// constructs; testAccConfig() returns it for tests that need to
		// reach the API client directly.
		config, err := testAccConfig()
		if err != nil {
			return fmt.Errorf("Error obtaining test Config: %w", err)
		}
...
// testAccComputeV2InterfaceAttachConflict deliberately sets both port_id and
// network_id, which the per-attribute ConflictsWith validators must reject at
// validate-time (PlanOnly).
func testAccComputeV2InterfaceAttachConflict() string {
	return fmt.Sprintf(`
resource "openstack_networking_port_v2" "port_1" {
  name = "port_1"
  network_id = "%s"
