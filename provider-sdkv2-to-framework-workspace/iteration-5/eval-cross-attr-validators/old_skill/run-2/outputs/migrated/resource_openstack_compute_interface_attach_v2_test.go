...
// TestAccComputeV2InterfaceAttach_conflictsPortNetwork asserts that the
// framework ConflictsWith validator on port_id <-> network_id fires at config
// validation time (i.e. without ever calling the API). This is the framework
// idiom that replaces the SDKv2 ConflictsWith schema field.
func TestAccComputeV2InterfaceAttach_conflictsPortNetwork(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
			testAccPreCheckNonAdminOnly(t)
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`(?s)Attribute "port_id" cannot be specified when "network_id"|Attribute "network_id" cannot be specified when "port_id"`),
				PlanOnly:    true,
			},
		},
	})
...
// TestAccComputeV2InterfaceAttach_conflictsFixedIPPort asserts the second
// ConflictsWith pair: fixed_ip <-> port_id.
func TestAccComputeV2InterfaceAttach_conflictsFixedIPPort(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
			testAccPreCheckNonAdminOnly(t)
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`(?s)Attribute "fixed_ip" cannot be specified when "port_id"|Attribute "port_id" cannot be specified when "fixed_ip"`),
				PlanOnly:    true,
			},
		},
	})
...
// network_id, which the framework's stringvalidator.ConflictsWith should
// reject at config-validation time.
...
// testAccComputeV2InterfaceAttachConflictFixedIPPort sets fixed_ip together
// with port_id, which the framework's ConflictsWith validator on fixed_ip
// should reject.
