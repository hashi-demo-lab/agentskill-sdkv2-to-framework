...
func testAccCheckObjectStorageV1ContainerDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// testAccProvider is the SDKv2 provider; for the migrated framework
		// resource, the destroy check still uses the provider Config from the
		// shared test setup since the gophercloud client is constructed the
		// same way.
