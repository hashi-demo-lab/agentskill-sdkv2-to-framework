...
## Configuration
- Skill: `old_skill` (snapshot at `<repo>/provider-sdkv2-to-framework-workspace/skill-snapshot-i4`)
- Provider repo (read-only): `<openstack-clone>`
- Output dir: `<repo>/provider-sdkv2-to-framework-workspace/iteration-5/eval-mux-refusal/old_skill/run-2/outputs`
...
The user's prompt (verbatim): *"migrate my Terraform provider at <openstack-clone> off SDKv2 using **terraform-plugin-mux** for a **staged, multi-release migration** — please help me set up the muxed provider so I can ship a few resources on the framework while keeping the rest on SDKv2 across **two release cycles**."*
