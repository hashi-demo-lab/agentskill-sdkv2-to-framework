...
//
// Migration notes:
//   - ProviderFactories -> ProtoV6ProviderFactories (the rsyslog logsink resource
//     is now served by terraform-plugin-framework via protocol v6).
//   - terraform-plugin-sdk/v2/helper/resource and .../terraform are replaced by
//     terraform-plugin-testing/helper/resource and .../terraform.
//   - acceptance.TestAccProtoV6ProviderFactories is the framework-served factory
//     map; it lives alongside the legacy TestAccProviderFactories so that
//     un-migrated resources keep working in the same test binary (typical
//     during a single-release migration that touches one resource at a time).
...
func TestAccDigitalOceanDatabaseLogsinkRsyslog_Basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Round-trip the legacy CLI import path (composite-ID string).
			{
				ResourceName:      "digitalocean_database_logsink_rsyslog.test",
				ImportState:       true,
				ImportStateVerify: true,
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					rs, ok := s.RootModule().Resources["digitalocean_database_logsink_rsyslog.test"]
					if !ok {
						return "", fmt.Errorf("resource not found in state")
					}
					return fmt.Sprintf("%s,%s",
						rs.Primary.Attributes["cluster_id"],
						rs.Primary.Attributes["logsink_id"],
					), nil
				},
			},
		},
...
//
// Migration note: the SDKv2 validation.NoZeroValues + custom validateLogsinkPort produced
// "must be between 1 and 65535". int64validator.Between formats its message as
// "must be between 1 and 65535" too — keep the regex, but if the framework wording
// drifts in a future release this is the assertion to revisit.
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile("(?i)between 1 and 65535"),
			},
		},
	})
}
...
//
// Migration note: stringvalidator.OneOf phrases failure as
// "Attribute format value must be one of: [\"rfc5424\" \"rfc3164\" \"custom\"], got: \"invalid_format\"".
// The original regex looked for the exact SDKv2 string "must be one of: rfc5424, rfc3164, custom".
// Loosen the regex so it matches the framework's wording without losing intent.
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`(?s)rfc5424.*rfc3164.*custom`),
			},
		},
	})
}
...
func testAccCheckDigitalOceanDatabaseLogsinkExists(resourceName string, logsink *godo.DatabaseLogsink) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		// Parse composite ID via the schema attributes (which the framework
		// resource exposes the same way SDKv2 did).
