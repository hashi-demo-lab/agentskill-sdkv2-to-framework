...
//
// This file migrates only the resource — provider-level mux (so the framework
// resource is actually served) is a separate piece of work. Once the provider
// is muxed (or fully migrated) the ProviderFactories field below should be
// replaced with ProtoV6ProviderFactories pointing at the framework provider:
//
//   ProtoV6ProviderFactories: map[string]func() (tfprotov6.ProviderServer, error){
//       "openstack": providerserver.NewProtocol6WithError(NewOpenStackFrameworkProvider()),
//   },
//
// Until then the test cases below continue to use the existing SDKv2-style
// `testAccProviders` map so the rest of the suite keeps working. The new
// `_importLegacyComposite` step (and the file-level identity-import comments)
// are still meaningful: when the provider is muxed they begin exercising the
// framework code paths automatically.
...
func TestAccLBV2Member_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Legacy CLI-style import path: <pool_id>/<member_id>. The framework
			// resource's ImportState handles req.ID parsing identically to the
			// SDKv2 importer it replaced.
			{
				ResourceName:            "openstack_lb_member_v2.member_1",
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateIdFunc:       testAccLBV2MemberImportStateIDFunc("openstack_lb_member_v2.member_1"),
...
// TestAccLBV2Member_importIdentity is a placeholder for a future test that
// exercises the modern Terraform 1.12+ `import { identity = {...} }` block.
//
// Driving this with terraform-plugin-testing requires:
//   1. Provider-level mux/migration so the framework resource is actually
//      served (right now the suite still uses ProviderFactories with the
//      SDKv2 provider).
//   2. The terraform-plugin-testing helpers `ImportStateKind:
//      ImportBlockWithResourceIdentity` plus `statecheck.ExpectIdentity` /
//      `ExpectIdentityValueMatchesState` (testing v1.13+).
//
// The function body is intentionally a no-op skip so the test compiles in
// every branch of the migration. It will be wired up once the provider mux
// step lands.
func TestAccLBV2Member_importIdentity(t *testing.T) {
	t.Skip("identity-driven import requires provider-level mux/migration; see notes.md")
}
...
// testAccLBV2MemberImportStateIDFunc renders the legacy <pool_id>/<member_id>
// composite import string from the prior state so the SDKv2-style CLI import
// path is exercised by the migrated framework resource.
func testAccLBV2MemberImportStateIDFunc(name string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		if poolID == "" || memberID == "" {
			return "", errors.New("pool_id or member id missing from state for import")
		}
