...
// TestAccLBV2Member_basic exercises Create/Update plus the legacy composite-ID
// importer (`terraform import openstack_lb_member_v2.foo <pool>/<member>`).
//
// Migrated to the framework: ProtoV6ProviderFactories replaces ProviderFactories.
// `protoV6ProviderFactories` is defined in the test scaffolding (provider_test.go)
// and points at providerserver.NewProtocol6WithError(...) wrapping the framework
// provider.
func TestAccLBV2Member_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Legacy composite-ID importer (works on every Terraform CLI version).
			// Asserts the parser still accepts `<pool>/<member>` and that the resulting
			// state round-trips clean.
			{
...
// TestAccLBV2Member_identityImport exercises the modern `import { identity = {...} }`
// path. Requires Terraform 1.12+ (the version that ships `import { identity }`
// support in plan-time).
//
// statecheck.ExpectIdentityValue asserts each identity attribute is populated
// after Create — proving CRUD writes to resp.Identity correctly.
func TestAccLBV2Member_identityImport(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Round-trip via identity. ImportStateVerify replays the prior state
			// after the import and asserts they match — proves Read populates
			// from (pool_id, id) alone, which is what the modern import path
			// delivers.
			{
				ResourceName:                         "openstack_lb_member_v2.member_1",
				ImportState:                          true,
...
// testAccLBV2MemberImportStateIdFunc returns the legacy composite ID format
// (`<pool>/<member>`) so the legacy importer is exercised under
// ImportStateVerify.
func testAccLBV2MemberImportStateIdFunc(name string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		if poolID == "" {
			return "", errors.New("pool_id missing from state")
		}
