...
// TestAccLBV2Member_basic exercises Create, Update, and tag handling. The
// migration switches the test from the SDKv2 `ProviderFactories` path to the
// framework `ProtoV6ProviderFactories` path so that the test boots the
// migrated, framework-based resource.
		...
		Steps: []resource.TestStep{
			{
				...
				// Identity-aware state assertion: the migrated resource exposes a
				// typed identity (pool_id, member_id) populated during Create.
...
// TestAccLBV2Member_importBasic verifies the legacy composite-ID import path
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				// timeouts is nested-attribute-only state and not round-trippable
				// through the legacy importer; ignore it on import-verify.
...
// TestAccLBV2Member_importIdentity verifies the modern Terraform 1.12+
// `import { identity = { pool_id = ..., member_id = ... } }` flow.
//
// The framework dispatches this to the resource's ImportState method with
// `req.ID == ""` and `req.Identity` populated; our ImportState reads the
// identity and writes pool_id + id into state.
