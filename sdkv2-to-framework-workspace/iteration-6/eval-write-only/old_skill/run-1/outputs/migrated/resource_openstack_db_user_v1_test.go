...
func TestAccDatabaseV1User_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// NOTE on factories: the openstack_db_user_v1 resource is now
		// implemented with terraform-plugin-framework, but the rest of
		// the provider remains on terraform-plugin-sdk/v2. The framework
		// resource is wired into the provider via plugin-mux at the
		// provider entry point (out of scope for this single-resource
		// migration). The existing testAccProviders factory continues
		// to drive the test from the practitioner's POV.
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// `password` is now WriteOnly: it must be null in
					// state regardless of what the practitioner passed
					// in config. Asserting absence here protects us from
					// accidentally re-introducing state persistence.
					resource.TestCheckNoResourceAttr(
						"openstack_db_user_v1.basic", "password"),
				),
			},
			{
				ResourceName:      "openstack_db_user_v1.basic",
				ImportState:       true,
				ImportStateVerify: true,
				// `password` is WriteOnly — it isn't present in state
				// after import, so ImportStateVerify must skip it or it
				// will fail comparing a null state value against the
				// config value.
