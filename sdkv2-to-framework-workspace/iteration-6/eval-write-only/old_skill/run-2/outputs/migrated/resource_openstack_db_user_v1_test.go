...
func TestAccDatabaseV1User_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// Migrated to plugin-framework: serve via protocol v6.
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// password is WriteOnly: assert it is NOT in state.
			...
			},
			{
				...
				// password is WriteOnly — not persisted to state, so import
				// readback would mismatch the original config value. Required
				// pairing per references/sensitive-and-writeonly.md.
