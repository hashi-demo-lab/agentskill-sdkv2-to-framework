...
func TestAccDatabaseV1User_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		// because the resource is now framework-based and the provider is
		// served via protocol v6 (or muxed onto v6 alongside any remaining
		// SDKv2 resources). Adjust the factory name to match whatever the
		// provider's test harness exposes once muxing is complete.
		...
		Steps: []resource.TestStep{
			...
			{
				// Import-verify step. The password attribute is WriteOnly —
				// it never appears in state, so ImportStateVerify would
				// otherwise compare config-side "password" against an absent
				// import-side value and fail. ImportStateVerifyIgnore tells
				// the harness to skip that comparison.
