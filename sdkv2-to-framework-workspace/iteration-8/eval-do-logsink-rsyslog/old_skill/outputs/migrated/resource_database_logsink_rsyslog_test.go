...
func TestAccDigitalOceanDatabaseLogsinkRsyslog_Basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy:             testAccCheckDigitalOceanDatabaseLogsinkRsyslogDestroy,
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(testAccCheckDigitalOceanDatabaseLogsinkRsyslogConfigBasic, clusterName, logsinkName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDigitalOceanDatabaseLogsinkRsyslogExists("digitalocean_database_logsink_rsyslog.test", &logsink),
					testAccCheckDigitalOceanDatabaseLogsinkRsyslogAttributes(&logsink, logsinkName, "rsyslog"),
...
// TestAccDigitalOceanDatabaseLogsinkRsyslog_Import tests import of a logsink via
// the composite ID (cluster_id,logsink_id). Expected: import succeeds and state matches.
func TestAccDigitalOceanDatabaseLogsinkRsyslog_Import(t *testing.T) {
	clusterName := acceptance.RandomTestName()
	logsinkName := acceptance.RandomTestName()
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				// The import ID is the composite "cluster_id,logsink_id" value stored in "id".
...
// testAccCheckDigitalOceanDatabaseLogsinkRsyslogExists fetches the logsink from the API and
// verifies it exists, populating the logsink pointer for further assertions.
func testAccCheckDigitalOceanDatabaseLogsinkRsyslogExists(resourceName string, logsink *godo.DatabaseLogsink) resource.TestCheckFunc {
...
// testAccCheckDigitalOceanDatabaseLogsinkRsyslogAttributes asserts that the logsink has
// the expected name and sink type.
func testAccCheckDigitalOceanDatabaseLogsinkRsyslogAttributes(logsink *godo.DatabaseLogsink, name, sinkType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
// testAccCheckDigitalOceanDatabaseLogsinkRsyslogDestroy verifies the logsink no longer exists.
func testAccCheckDigitalOceanDatabaseLogsinkRsyslogDestroy(s *terraform.State) error {
	client := acceptance.TestAccProvider.Meta().(*config.CombinedConfig).GodoClient()

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "digitalocean_database_logsink_rsyslog" {
			continue
		}
		...
		if err == nil {
			return fmt.Errorf("DatabaseLogsinkRsyslog still exists")
		}
	}
