		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// password is Sensitive + Computed (NOT WriteOnly): the API returns
					// it on create and Terraform stores it in state for downstream
					// references. TestCheckResourceAttrSet works normally for Sensitive
					// attributes — the value is in state, just redacted from logs.
					resource.TestCheckResourceAttrSet(
...
func TestAccDigitalOceanDatabaseUser_MongoDBMultiUser(t *testing.T) {
	...
	cfg := fmt.Sprintf(testAccCheckDigitalOceanDatabaseUserConfigMongoMultiUser,
		databaseClusterName,
		users[0], users[0],
		users[1], users[1],
		users[2], users[2],
