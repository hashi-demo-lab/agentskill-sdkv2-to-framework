...
// All test cases below are direct ports of the SDKv2 versions. Two changes:
//
//  1. ProviderFactories -> ProtoV6ProviderFactories. The migrated provider is
//     served via providerserver.NewProtocol6WithError; protoV6ProviderFactories
//     is expected to be defined in the package's test helpers (provider_test.go).
//
//  2. The "import" subtest now uses ImportStateVerifyIgnore for `value` and
//     `key_id` because the framework round-trips state more strictly than SDKv2,
//     and `value` (plaintext) is not recoverable from GitHub on import.
//     `repository_id`, `created_at`, `updated_at`, and `remote_updated_at` are
//     populated by Read post-import, so they do NOT need to be ignored.
//
// Per references/testing.md (TDD ordering, step 7): this file is intended to be
// updated and run RED *before* the resource implementation switches to the
// framework. The expected red-state failure mode is a compile error citing
// `providerFactories` (SDKv2 type) or a protocol-mismatch panic at runtime.
