...
// Migrated test file: terraform-plugin-sdk/v2 → terraform-plugin-framework
//
// Changes from the original:
//   - ProviderFactories → ProtoV6ProviderFactories (protoV6ProviderFactories).
//   - Acceptance test structure and HCL configs are otherwise unchanged.
//   - The state-upgrade unit test (previously in
//     migrate_resource_openstack_objectstorage_container_v1_test.go) is rewritten
//     using framework-typed state helpers instead of raw map[string]any.
//     This provides compile-time type safety: a mismatch between PriorSchema and
//     the V0 model struct tags will fail at Get() rather than silently losing data.
...
// TestAccObjectStorageV1Container_importBasic exercises the import round-trip.
...
// ---------------------------------------------------------------------------
// State-upgrade unit test
// ---------------------------------------------------------------------------

// TestObjectStorageV1ContainerStateUpgradeV0 is a non-acceptance unit test that
// directly exercises upgradeContainerStateV0toV1.
//
// It builds a fake V0 state using the framework's tfsdk.State helpers, calls the
// upgrader, and asserts the resulting V1 state has the expected field values.
//
// Unlike the original SDKv2 test (which called resourceObjectStorageContainerStateUpgradeV0
// on a raw map[string]any), this test is fully typed — a mismatch between the
// PriorSchema and the V0 model struct tags causes a test failure at Get() rather
// than silent data loss.
func TestObjectStorageV1ContainerStateUpgradeV0(t *testing.T) {
	ctx := context.Background()

	// Build the V0 prior state using containerSchemaV0().
	priorSchema := containerSchemaV0()

	// Construct the raw tftypes.Value matching the V0 schema.
	...
	if got := result.VersioningLegacy[0].Type.ValueString(); got != "versions" {
		t.Errorf("expected versioning_legacy[0].type=versions, got %q", got)
	}
	if got := result.VersioningLegacy[0].Location.ValueString(); got != "test" {
		t.Errorf("expected versioning_legacy[0].location=test, got %q", got)
	}
	...
	if got := result.Region.ValueString(); got != "RegionOne" {
		t.Errorf("expected region=RegionOne, got %q", got)
	}

	// force_destroy must be false.
	if result.ForceDestroy.ValueBool() {
		t.Errorf("expected force_destroy=false, got true")
	}

	// Ensure types package reference is used (import guard).
	_ = types.StringNull()
}
