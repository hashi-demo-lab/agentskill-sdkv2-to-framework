		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// Defaults exposed via stringdefault.* should land in state.
...
// testAccCheckIKEPolicyV2Destroy uses the bare gophercloud client; provider
// state plumbing differs from SDKv2 (no testAccProvider.Meta()), so we drive
// it directly from the test config helpers.
func testAccCheckIKEPolicyV2Destroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		config, err := testAccConfig()
		if err != nil {
			return fmt.Errorf("Error retrieving test provider config: %w", err)
		}
