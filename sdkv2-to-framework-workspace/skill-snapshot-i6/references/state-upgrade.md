...
- **Returning V1 from the V0 upgrader (chain habit)**: this leaves state two versions behind. The framework writes whatever the upgrader sets to `resp.State`; if your typed `Set` call uses the V1 model against the current (V2) schema, Go's typed reflection will surface a hard error from the framework, but if the V1 and current models share enough field names, you'll silently ship corrupt state with permanent planning diffs. Either way, the resource never reaches a stable plan.
